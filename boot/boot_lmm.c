/**
 * @file boot/boot_lmm.c
 * @brief Boot LMM setup
 *
...
void boot_lmm (mbinfo_t *mbi, lmm_t *lmm) {
	vm_offset_t min;
	...
	// Initialize the base memory allocator according to physical memory regions
	lmm_init(lmm);

  // Build our initial regions
  ...
  lmm_add_region(&malloc_lmm, &reghigh,
	...
	do {
		...
		if ((mbi->flags & MULTIBOOT_MODS) && (mbi->mods_count > 0)) {
